...
description: >
  ...
  Tests run against a real database (Docker/Testcontainers) and real HTTP server. Use whenever
  the user wants tests that need live infrastructure — not mocks. Trigger on: "tạo integration test",
  "viết integration test", "test endpoint API", "test repository với database thật",
  "test toàn bộ luồng", "WebApplicationFactory", "Testcontainers", or any request for tests
  that require a real database or actual HTTP calls.
  Do NOT trigger for pure unit tests with mocked dependencies — use dotnet-clean-unit-test instead.
  Also invoke during /speckit.tasks or /speckit.implement for tasks under
  tests/{Project}.IntegrationTests/ or tests/{Project}.FunctionalTests/.
metadata:
  related-skills:
