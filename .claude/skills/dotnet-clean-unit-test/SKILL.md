...
description: >
  ...
  "tạo unit test", "viết unit test", "thêm test cho entity", "tạo test project",
  "test command handler", "test query handler", "test domain entity", "mock dependency",
  "NSubstitute", "viết test không cần database".
  Do NOT trigger for integration tests, API tests, or WebApplicationFactory-based tests,
  or when the user mentions database, Docker, Testcontainers, or real HTTP calls —
  use dotnet-clean-integration-test instead. Also do NOT trigger on bare "viết test"
  without context — ask the user whether they want unit or integration tests.
  Also invoke during /speckit.tasks or /speckit.implement for tasks under
  tests/{Project}.Domain.UnitTests/ or tests/{Project}.Application.UnitTests/,
  or mentioning unit tests, xUnit, or NSubstitute.
metadata:
  related-skills:
...
|-------|-------------|---------|
| **Domain** | Entity behavior, domain logic, domain exceptions | No mocking needed — pure logic |
...
- `references/domain-test-templates.md` — Entity tests, domain exception tests
...
- **`sealed` vs non-`sealed` in tests** — test classes themselves are never `sealed` (xUnit requires
  class inheritance for fixtures). When testing handlers that are `internal sealed class`, test them
  via their public interface — NSubstitute substitutes the *interface*, not the sealed class directly.
  Validators (`internal class`, non-sealed) can be tested directly by instantiating them.
