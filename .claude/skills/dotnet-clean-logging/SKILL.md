...
description: >
  ...
  (destructure object to JSON), and exception logging patterns.
  Trigger whenever the user wants to add log lines, instrument code with logs,
  log a request/response, add logging to a service/handler/middleware/background
  service, debug with structured logs, or set up observability — including
  Vietnamese like "thêm logging", "ghi log cho service", "log request response",
  "thêm log vào controller", "thêm log cho background service", "log pipeline
  behavior", "thêm observability", "ghi log exception", "log middleware".
  Do NOT trigger when the user is only asking what logging is conceptually,
  setting up Serilog sinks/configuration (that belongs in scaffold/config),
  or adding distributed tracing with OpenTelemetry.
  Also invoke during /speckit.tasks or /speckit.implement whenever any task involves
  a handler, service, or endpoint — always pair a logging task alongside those tasks.
metadata:
  related-skills:
...
## [TraceLog] Attribute — AOP Logging

For **cross-cutting trace logging** (enter/exit/error on methods), the project provides a `[TraceLog]` attribute powered by **AspectInjector**. Use this instead of writing `LogInformation("Entering...")` boilerplate by hand.
...
The aspect automatically emits:
- `"Entering method {FullTypeName}.{MethodName}"` on entry
- `"Exiting method {FullTypeName}.{MethodName}"` on successful exit (sync and async)
- `"Error in method {FullTypeName}.{MethodName}: {ExceptionMessage}"` on exception, then re-throws

### Optional properties

```csharp
[TraceLog("Processing payment")]          // custom message label
[TraceLog(LogParameters = false)]         // suppress parameter logging
[TraceLog(LogReturnValue = false)]        // suppress return value logging
```

### When to use [TraceLog] vs manual ILogger\<T>

| Situation | Use |
|---|---|
| Trace entry/exit of a handler, service method, or repository call | `[TraceLog]` |
| Business event with specific data ("Order {OrderId} created for {CustomerId}") | `ILogger<T>` manually |
| Exception with full context and stack trace | `ILogger<T>` manually — `[TraceLog]` only logs `ex.Message` |
| Hot path (called thousands of times per second) | `ILogger<T>` with `LogDebug` — AOP has slight overhead |

### Targets supported

`AttributeTargets.Method | Property | Constructor` — works on all three. Most common use case: `Method`.

### Note on Inherited = false

`[TraceLog]` is not inherited. If a base class method is overridden, apply the attribute to the override explicitly.
